...
public class MoveToController : MonoBehaviour
{
    ...
    bool approach =false;
    int m_nowTargetNumber = 0;
    [SerializeField] float m_timeout = 0.5f;
    float m_timer = 0;
    bool hit = false;
    ...
    void MoveToTarget0()
    {
        ...
        if (distance > m_stoppingDistance)  // ターゲットに到達するまで処理する
        {
            ...
            if (approach)
            {
                Vector3 dir1 = (m_targets[1].transform.position - this.transform.position).normalized * m_moveSpeed;
    ...
    void Patrol()
    {
        float distance = Vector2.Distance(this.transform.position, m_targets[m_nowTargetNumber].position);
        if (distance > m_stoppingDistance)
        {
            Vector3 dir = (m_targets[m_nowTargetNumber].transform.position - this.transform.position).normalized * m_moveSpeed; // 移動方向のベクトルを求める
            this.transform.Translate(dir * Time.deltaTime); // Update の中で移動する場合は、Time.deltaTime をかけることによりどの環境でも同じ速さで移動させることができる
        }
        else
        {
            m_nowTargetNumber = (m_nowTargetNumber + 1) % m_targets.Length;
        }
    ...
    void PatrolWithChangeTargetByTimeout()
    {
        ...
        float distance = Vector2.Distance(this.transform.position, m_targets[m_nowTargetNumber].position);
        if (distance > m_stoppingDistance && m_timer < m_timeout)
        {
            Vector3 dir = (m_targets[m_nowTargetNumber].transform.position - this.transform.position).normalized * m_moveSpeed; // 移動方向のベクトルを求める
            this.transform.Translate(dir * Time.deltaTime); // Update の中で移動する場合は、Time.deltaTime をかけることによりどの環境でも同じ速さで移動させることができる
        }
    ...
    {
        float distance = Vector2.Distance(this.transform.position, m_targets[m_nowTargetNumber].position);
        if (distance > m_stoppingDistance && !hit)
        {
